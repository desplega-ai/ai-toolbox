...
def validate_path(file_path: str) -> tuple[bool, str]:
    ...
    # Plans accept two shapes:
    #   - linear (planning skill):   thoughts/{user|shared}/plans/YYYY-MM-DD-topic.md
    ...
    if "/research/" in file_path:
    ...
    elif "/brainstorms/" in file_path:
        if not re.search(brainstorms_pattern, file_path):
            return False, (
                "Invalid brainstorm path format.\n"
                "Expected: thoughts/{username|agentId|shared}/brainstorms/YYYY-MM-DD-topic-slug.md\n"
                f"Got: {file_path}"
            )
        ...
        if not re.search(learnings_pattern, file_path):
            return False, (
                "Invalid learnings path format.\n"
